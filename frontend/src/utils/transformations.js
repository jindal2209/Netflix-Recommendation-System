...
export const genreTopVideoTransformation = async (genres, apiCallType) => {
    let url = `${process.env.REACT_APP_TMDB_BASE_URL}/discover/movie?api_key=${process.env.REACT_APP_MOVIEDB_API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=`
...
export const mediaTypeToVideoDetailTransformation = async (videoId, mediaType) => {
    let requestURL = `${process.env.REACT_APP_TMDB_BASE_URL}/movie/${videoId}?api_key=${process.env.REACT_APP_MOVIEDB_API_KEY}&language=en-US`
