...
const SearchContent = props => {
    ...
    const getSearchMovies = async (searchItem) => {
        const movieUrl = `${process.env.REACT_APP_TMDB_BASE_URL}/search/movie?api_key=${process.env.REACT_APP_MOVIEDB_API_KEY}&language=en-US&query=${searchItem}&page=1&include_adult=false`
        ...
        try {
            ...
            setSearchedVideoList([...responses[0]])
            setLoading(false)
        } catch (error) {
